...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (it == NULL) {
		printf("[do_item_alloc] item alloc failed!\n");
        } else {
            break;
        }
    }
...
static int lru_pull_tail(const int orig_id, const int cur_lru,
        const unsigned int total_chunks, const bool do_evict, const uint32_t cur_hv) {
    printf("[lru_pull_tail] You can have to review here\n");
    item *it = NULL;
    int id = orig_id;
    int removed = 0;
    if (id == 0)
...
static void *item_crawler_thread(void *arg) {
	printf("[item_crawler_thread] You can have to review here\n");
    int i;
