...
void crash_prefix(art_node *n) {
    ...
    for (i=0; i < 16; i++) {
         if (!((art_node16*)n)->children[i]) continue;
...
void recover_recursive_iter_PREFIX(art_node *n, int depth) {
    if (!n) return;
    if (IS_LEAF(n)) return;
    ...
    for (i=0; i < 16; i++) {
        if (!((art_node16*)n)->children[i]) continue;
        recover_recursive_iter_PREFIX(((art_node16*)n)->children[i], depth + n->path.partial_len + 1);
    }
...
void recover_tree(art_tree *t) {
	recover_recursive_iter_PREFIX(t->root, 0);
	return;
}
