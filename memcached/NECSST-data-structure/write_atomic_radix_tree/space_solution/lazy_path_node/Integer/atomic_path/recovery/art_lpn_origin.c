...
void recover_tree(art_tree *t) {
	return recover_recursive_iter(t->root, 0);
}

static void recover_recursive_iter(art_node *n, int depth) {
    if (!n) return;
    if (IS_LEAF(n)) return;
    ...
    switch (n->type) {
        case NODE4:
            for (i=0; i < n->num_children; i++) {	//[kh] It should be granteed that the num_children is two at least. (delete가라vers. 주의)
				...
				int prefix_diff = longest_common_prefix(l1, l2, depth);

				if (n->path.partial_len != prefix_diff) {
					path_comp *path_temp = calloc(1, sizeof(path_comp));
					memcpy(*path_temp, l1->key, min(prefix_diff, MAX_PREFIX_LEN));
				...
				depth += n->path.partial_len;
                recursive_iter(((art_node4*)n)->children[i], depth+1);
            }
            break;

        case NODE16:
            for (i=0; i < n->num_children; i++) {
				art_leaf *l1 = minimum(((art_node16*)n)->children[0]);
				art_leaf *l2 = minimum(((art_node16*)n)->children[1]);
				int prefix_diff = longest_common_prefix(l1, l2, depth);
				...
				depth += n->path.partial_len;
                recursive_iter(((art_node16*)n)->children[i], depth+1);
            }
            break;
        ...
        case NODE48:
            for (i=0; i < 256; i++) {
				int cnt;
				int pos;
				for (pos = 0, cnt = 0; pos < 48 && cnt < 2; pos++) {
					if (!n->children[pos]) {
                ...
                if (!idx) continue;
                recursive_iter(((art_node48*)n)->children[idx-1], depth+1);
            }
            break;
        ...
        case NODE256:
            for (i=0; i < 256; i++) {
                ...
                recursive_iter(((art_node256*)n)->children[i], depth+1);
            }
            break;
