...
static art_leaf* make_leaf(const unsigned long key, int key_len, void *value) {
	...
	art_leaf *l;
	posix_memalign((void *)&l, 64, sizeof(art_leaf));
	l->value = value;
	l->key_len = key_len;
	l->key = key;
	leaf_count++;
...
void recover_recursive_iter(art_node *n, int depth) {
    if (!n) return;
    if (IS_LEAF(n)) return;
    ...
    switch (n->type) {
        case NODE4:
            for (i=0; i < n->num_children; i++) {	//[kh] It should be granteed that the num_children is two at least. (delete가라vers. 주의)
                recover_recursive_iter(((art_node4*)n)->children[i], depth + n->path.partial_len + 1);
			}
            break;

        case NODE16:
            for (i=0; i < n->num_children; i++) {
                recover_recursive_iter(((art_node16*)n)->children[i], depth + n->path.partial_len + 1);
            }
            break;
        ...
        case NODE48:
            for (i=0; i < 256; i++) {
                idx = ((art_node48*)n)->keys[i];
                if (!idx) continue;
                recover_recursive_iter(((art_node48*)n)->children[idx-1], depth + n->path.partial_len + 1);
            }
            break;
        ...
        case NODE256:
            for (i=0; i < 256; i++) {
                if (!((art_node256*)n)->children[i]) continue;
                recover_recursive_iter(((art_node256*)n)->children[i], depth + n->path.partial_len + 1);
            }
            break;
...
void recover_tree(art_tree *t) {
	recover_recursive_iter(t->root, 0);
}
/**
