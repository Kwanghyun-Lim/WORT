...
//	printf("[item_lock] hv:%u | item_locks[hv:%u & hashmask(item_lock_hashpower:%u)]:%lu\n", hv, hv, item_lock_hashpower, hv & hashmask(item_lock_hashpower));
}
...
//		printf("[item_trylock] hv:%u | item_locks[hv:%u & hashmask(item_lock_hashpower:%u)]:%lu\n", hv, hv, item_lock_hashpower, hv & hashmask(item_lock_hashpower));
        return lock;
    }
    return NULL;
}
...
//	mutex_unlock(&T_lock);
//	printf("[item_trylock_unlock] void *lock | void\n");
}

void item_unlock(uint32_t hv) {
    mutex_unlock(&item_locks[hv & hashmask(item_lock_hashpower)]);
	mutex_unlock(&T_lock);
//	printf("[item_unlock] hv:%u | item_locks[hv:%u & hashmask(item_lock_hashpower:%u)]:%lu\n", hv, hv, item_lock_hashpower, hv & hashmask(item_lock_hashpower));
}
 ...
 */
int item_link(item *item) {    // [assoc_insert] 
    int ret;
    uint32_t hv;
	printf("[item_link]\n");
 ...
 */
int item_replace(item *old_it, item *new_it, const uint32_t hv) { // [assoc_insert]
 ...
 */
enum store_item_type store_item(item *item, int comm, conn* c) { // [assoc_insert]
...
//#ifdef KH_THREAD
	printf("[memcached_thread_init] nthreads : %d, power : %u(item_lock_count:%u), hashpower:%u | nothing yet\n", nthreads, power, item_lock_count, hashpower);
//#endif
