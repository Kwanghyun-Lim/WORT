...
//	if (hv == 360372863) {
//		printf("[[do_store_item]] just before if hv:%u, ITEM_key(it):%s, old_it:%p, comm:%d\n", hv, key, (void *)old_it, comm);
//	}

    if (old_it != NULL && comm == NREAD_ADD) {
        /* add only adds a nonexistent item, but promote to head of LRU */
//	printf("[[do_store_item]] just before do_item_update(old_it)\n");
	...
	//	printf("[[do_store_itme]] just_before do_item_link\n");
                do_item_link(it, hv);
	    }
            c->cas = ITEM_get_cas(it);
    ...
    if (old_it != NULL) {
//	printf("[[do_store_item]] just before do_item_remove(old_it)\n");
        do_item_remove(old_it);         /* release our reference */
    }
    if (new_it != NULL) {
//	printf("[[do_store_item]] just before do_item_remove(new_it)\n");
        do_item_remove(new_it);
    }

    if (stored == STORED) {
	//printf("[[do_store_item]] just before c->cas==ITEM_get_cat(it)\n");
        c->cas = ITEM_get_cas(it);
    }
//	printf("[[do_store_item]] stored: %d\n",stored);
    return stored;
}
