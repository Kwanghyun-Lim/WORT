...
void* art_search(const art_tree *t, const unsigned char *key, int key_len) {
    ...
    while (n) {
        ...
        if (n->partial_len) { //if (n->partial_len 
		...
		//	struct path;
		//	path.partial_len = prefix_diff;
		//	int i;
		//	for (i = 0; i < min(MAX_PREFIX_LEN, prefix_diff); i++)
		//		path.partial[i] = get_index(leaf[1]->key, depth + i);
		//
		//	
		//
		//
...
static void* recursive_insert(art_node *n, art_node **ref, const unsigned char *key, int key_len, void *value, int depth, int *old) {
    ...
    if (n->partial_len) {
        ...
        *ref = (art_node*)new_node;
		...
		//n->n.path.depth = new_node->n.path.partial_len + 1;
		//buffer_flush(n->n.path, 8, true);
