...
/*
    ...
    if (n->n.num_children == 1) {
        ...
        if (!IS_ITEM(child)) {
            ...
            int prefix = n->n.path.partial_len;
            if (prefix < MAX_PREFIX_LEN) {
                n->n.path.partial[prefix] = n->keys[0];
                prefix++;
            }
            if (prefix < MAX_PREFIX_LEN) {
                int sub_prefix = min(child->path.partial_len, MAX_PREFIX_LEN - prefix);
                memcpy(n->n.path.partial+prefix, child->path.partial, sub_prefix);
                prefix += sub_prefix;
            }

            // Store the prefix in the child
            memcpy(child->path.partial, n->n.path.partial, min(prefix, MAX_PREFIX_LEN));
            child->path.partial_len += n->n.path.partial_len + 1;
        }
        *ref = child;
        free(n);
    }
