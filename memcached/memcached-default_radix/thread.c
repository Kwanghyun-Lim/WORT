 ...
 */
item *item_get(const char *key, const size_t nkey) {//ok
    item *it;
	tree_lock();
...
item *item_touch(const char *key, size_t nkey, uint32_t exptime) {//ok
    item *it;
	tree_lock();
 ...
 */
enum store_item_type store_item(item *item, int comm, conn* c) { //ok 
