...
void assoc_init(const int hashtable_init) {  /*[KH] The variable "hashtable_init" is not used as real */
	...
	printf("[assoc_init] init done 300w300r''\n");
	return;	
}
...
item *assoc_find(const char *_key, const size_t nkey) { //ok	//ok
	//printf("[assoc_find]\n");
     ...
     return (item *)current_LN->LN_Element[pos].value;
fail:
    return NULL;
}
...
int assoc_insert(item *it) {	//ok	//ok
	...
	return 1;  //[kh] I changed the reture value 0 -> 1 
fail:
	return errval;
}
...
void assoc_delete(const char *_key, const size_t nkey) {			//ok
	...
	memmove(&current_LN->LN_Element[pos], &current_LN->LN_Element[pos+1], (current_LN->nElements - (pos + 1))*sizeof(struct LN_entry));
...
LN *allocLNode(void)
{
//     LN *new_LN = malloc(sizeof(LN));
	 LN *new_LN;
	 posix_memalign((void *)&new_LN, 64, sizeof(LN));
	 new_LN->nElements = 0;
     LN_cnt++;
     return new_LN;
}
...
int leaf_split_and_insert(tree *t, LN *leaf, key_item *new_item, void *value)
{
	...
	if (t->is_leaf != 0) {
		...
		new_PLN->nKeys++;
		new_PLN->entries[new_PLN->nKeys].key = make_key_item((unsigned char *)MAX_KEY, strlen(MAX_KEY));
