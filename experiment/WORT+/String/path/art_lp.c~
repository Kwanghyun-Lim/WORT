...
/**
 * Macros to increase DRAM latency
 */
#define LATENCY			200
...
//	art_leaf *l = (art_leaf*)malloc(sizeof(art_leaf) + key_len);
	art_leaf *l;
	posix_memalign((void *)&l, 64, sizeof(art_leaf) + key_len);
	l->value = value;
	l->key_len = key_len;
...
//					min(MAX_PREFIX_LEN, n->partial_len));
			for (i = 0; i < min(MAX_PREFIX_LEN, n->partial_len); i++)
				n->partial[i] = get_partial(l->key, depth + prefix_diff + 1 + i);
		}
...
//	add_child((art_node16 *)n, ref, key[depth], SET_LEAF(l));
	add_child((art_node16 *)n, ref, get_partial(key, depth), SET_LEAF(l));
