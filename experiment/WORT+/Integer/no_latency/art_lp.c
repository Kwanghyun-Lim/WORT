		...
		int key_len, void *value, int depth, int *old)
{
	...
	if (IS_LEAF(n)) {
		...
		*ref = (art_node*)new_node;
		...
		//kh wort+
		new_node->cd.num_c = 2;
		insertion_sort(new_node->cd.c_arr, get_index(l->key, depth + longest_prefix), cd.num_c);
		insertion_sort(new_node->cd.c_arr, get_index(l2->key, depth + longest_prefix), cd.num_c);
