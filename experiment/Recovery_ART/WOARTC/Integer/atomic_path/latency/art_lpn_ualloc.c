...
char* user_malloc(int size)
{
   ...
   if(block==NULL|| (cur_offset+size) > 1024*1024*1024){
       block = (char*) pmalloc(1024*1024*1024);
       if(block==NULL){
           printf("user_alloc: failed.\n");
       }
       cur_offset=0;
   }

   ret = (char*) ((unsigned long long)block+cur_offset);
   cur_offset += size;

   return ret;
}

void user_free(char* ptr)
{

}

void user_pfree(int n,char* ptr)
{
...
static art_leaf* make_leaf(const unsigned long key, int key_len, void *value) {
    art_leaf *l = (art_leaf*)user_malloc(sizeof(art_leaf));
    l->value = value;
    l->key_len = key_len;
	l->key = key;
	leaf_count++;
...
void recover_recursive_iter(art_node *n, int depth) {
	...
	if (n->path.partial_len != prefix_diff) {
		...
		memcpy(path_temp, &leaf[1]->key, min(prefix_diff, MAX_PREFIX_LEN));
